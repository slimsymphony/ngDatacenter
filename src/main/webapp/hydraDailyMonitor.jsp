	...
	try {
		...
		if ( root.getName().equals( "notification" ) ) {
			...
			if ( ele != null && "release-updated".equalsIgnoreCase( ele.getStringValue() ) ) {
				...
				for(Element ele2 : els) {
					if("release-state".equalsIgnoreCase(ele2.element( "name" ).getStringValue())) {
						if("Ready".equalsIgnoreCase(ele2.element( "value" ).getStringValue())) {
							LogUtils.getWebLog().info( "Start daily hydra job!" );
							//CommonUtils.invokeRemote( "http://becim010:8007/job/Hydra-daily-release-getter-aqua_ds_target_F1/build?delay=0sec" );
							//CommonUtils.invokeRemote( "http://becim010:8007/job/Hydra-daily-release-getter-aqua_ss_target_F1/build?delay=0sec" );
							//CommonUtils.invokeRemote( "http://becim019.rnd.nokia.com:8080/job/Hydra-daily-release-getter-orionDS/build?delay=0sec" );
							//CommonUtils.invokeRemote( "http://becim019.rnd.nokia.com:8080/job/Hydra-daily-release-getter-orionSS/build?delay=0sec" );
							//CommonUtils.invokeRemote( "http://becim019.rnd.nokia.com:8080/job/Hydra-daily-release-getter-pegasusDS/build?delay=0sec" );
							//CommonUtils.invokeRemote( "http://becim019.rnd.nokia.com:8080/job/Hydra-daily-release-getter-pegasusSS/build?delay=0sec" );
							CommonUtils.invokeRemote( "http://becim019.rnd.nokia.com:8080/job/Hydra-daily-release-getter-ng14-pegasus/build?delay=0sec" );
						}else {
				...
				LogUtils.getWebLog().error( "Not a  release-updated event" );
			}
		} else {
